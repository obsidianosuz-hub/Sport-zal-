                ...
                <div class="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
                    ...
                    <div class="lg:col-span-1 bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                        <div class="p-4 border-b border-gray-100 bg-gray-50/50">
                            <h3 class="font-bold text-gray-800">Mijozlar Holati</h3>
                        </div>
                        <div class="divide-y divide-gray-100 max-h-[600px] overflow-y-auto">
                            <div v-for="history in histories" :key="'side-' + history.id" class="p-4 flex flex-col hover:bg-gray-50/50 transition-colors">
                                <div class="font-bold text-emerald-600 text-sm mb-1.5">{{ history.client?.name }}</div>
                                <div class="flex justify-between items-center text-xs">
                                    <div class="font-medium">
                                        <span class="text-gray-400">Kirgan:</span>
                                        <span class="text-gray-800 ml-1">{{ formatDate(history.arrived_at) }}</span>
                                    </div>
                                    <div class="font-medium">
                                        <span v-if="history.left_at" class="text-gray-600">{{ formatDate(history.left_at) }}</span>
                                        <span v-else class="text-blue-500 animate-pulse">Zalda</span>
                                    </div>
                                </div>
                            </div>
